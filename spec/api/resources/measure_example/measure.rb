...
class RotateBuilding < OpenStudio::Ruleset::ModelUserScript
  ...
  def arguments(model)
    ...
    relative_building_rotation.setDefaultValue(90)
  ...
  def run(model, runner, user_arguments)
    ...
    runner.registerInitialCondition("Your building's initial rotation is #{building.northAxis} degrees.")
    ...
    if model_contains_site_shading
      runner.registerWarning("The model contains one or more site shading groups. They are not rotated with the building.")
    end

    #reporting final condition of model
    runner.registerFinalCondition("Your building's final rotation is #{building.northAxis} degrees.")
